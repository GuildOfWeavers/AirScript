...
import {
    EvaluationContext, ExecutionContext, VerificationContext, ComputedRegister, ConstraintSpecs, ConstantSpecs
...
export class AirObject {
    ...
    createContext(pInputs: bigint[][], sInputs: bigint[][]): ExecutionContext;
    createContext(pInputs: bigint[][], sInputs?: bigint[][]): VerificationContext | ExecutionContext {
        ...
        let ctx: EvaluationContext, sRegisters: ComputedRegister[];
        if (sInputs) {
            const evaluationDomain = field.getPowerCycle(rootOfUnity);
            const executionDomain = new Array<bigint>(totalSteps);
    ...
    // EXECUTION
    // --------------------------------------------------------------------------------------------
    generateExecutionTrace(initValues: bigint[], ctx: ExecutionContext): bigint[][] {
        ...
        for (let register = 0; register < trace.length; register++) {
            trace[register] = new Array<bigint>(ctx.totalSteps);
    ...
    evaluateConstraints(trace: bigint[][], ctx: ExecutionContext) {
    ...
    // PRIVATE METHODS
    // --------------------------------------------------------------------------------------------
    private buildConstantRegisters(ctx: EvaluationContext) {
    ...
    private buildInputRegisters(inputs: bigint[][], ctx: EvaluationContext) {
