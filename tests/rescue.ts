...
define Rescue over prime field (2^64 - 21 * 2^30 + 1) {
    ...
    transition 2 register {
        for each (value1, value2) {
            ...
            for steps [1..31] {
                S <- MDS # $r^alpha + roundConstant1;
                yield MDS # (/S)^inv_alpha + roundConstant2;
            }
        }
    }
    ...
    enforce 2 constraint {
        for each (value1, value2) {
            ...
            for steps [1..31] {
                T1 <- MDS # $r^alpha + roundConstant1;
                T2 <- (INV_MDS # ($n - roundConstant2))^alpha;
                enforce T1 = T2;
            }
        }
    }

    static roundConstant1: [
        repeat [
